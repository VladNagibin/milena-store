...
export class ProductsService {
      ...
      Promise.allSettled([Pdata, PallFiles]).then(([data, allFiles]) => {
        if (data.status == 'rejected') {
          ...
          if (allFiles.status == 'fulfilled') {
            const pics = allFiles.value
              .filter((el) => el.isFile())
              .map((el) => `/pictures/${id}/${el.name}`);
            res({
              ...data.value,
              pics,
            });
  ...
  async create(data: NewProduct): Promise<Product> {
    const category = this.categoryRepository.findOneBy({ id: data.categoryId });
    const properties: Array<Promise<Property>> = [];
    const sizes: Array<Promise<Size>> = [];
    const colors: Array<Promise<Color>> = [];
    if (data.properties) {
    ...
    const newSizes = Promise.all(sizes);
    const newProps = Promise.all(properties);
    const newColors = Promise.all(colors);
    const [foundedCategory, savedSizes, savedProps, savedColors] =
      await Promise.all([category, newSizes, newProps, newColors]);
  ...
  async change(data: ChangeProduct): Promise<Product | string> {
    ...
    if (data.properties) {
      ...
      const propSavings: Array<Promise<Property | Error>> = [];
      data.properties.forEach((prop) => {
        propSavings.push(
          new Promise((res, rej) => {
    ...
    if (data.sizes) {
      ...
      const sizesSavings: Array<Promise<Size | Error>> = [];
      data.sizes.forEach((size) => {
        sizesSavings.push(
          new Promise((res, rej) => {
            this.sizeRepository
    ...
    if (data.colors) {
      ...
      const colorsSavings: Array<Promise<Size | Error>> = [];
      data.colors.forEach((color) => {
        colorsSavings.push(
          new Promise((res, rej) => {
