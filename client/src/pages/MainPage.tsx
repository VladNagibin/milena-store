...
export default function MainPage() {
  ...
  const getLatest = useCallback( async (signal:AbortSignal) =>{
      request<IProductDetailed[]>(`/products/latest`, 'GET', null, {}, signal).then(data => {
        setLatest(data)
          
      })
  },[])
  const getFavorite = useCallback( async (signal:AbortSignal) =>{
    request<IProductDetailed[]>(`/products/favorite`, 'GET', null, {}, signal).then(data => {
      setFavorite(data)
  ...
  return (
    <div>
      ...
      {!loading && favorite.length>=3?<><h2 style={{margin:"20px"}}>Хит продаж</h2><Slider {...settings}>
        {
        favorite.slice(0,10).map(product=>{
          return <Product product={product} key={product.id} settings={{slider:true}}/>
        })}
      </Slider></>:<></>}
      
      {!loading && latest.length>=3?<><h2 style={{margin:"20px"}}>Новинки</h2><Slider {...settings}>
        {
